...
\setlength{\cftbeforetoctitleskip}{-22pt} % Pull up to match Chapter top

\renewcommand{\cftloftitlefont}{\hfill\bfseries\fontsize{14pt}{17pt}\selectfont}
\renewcommand{\cftafterloftitle}{\hfill}
\setlength{\cftbeforeloftitleskip}{-22pt} % Pull up to match Chapter top

\renewcommand{\cftlottitlefont}{\hfill\bfseries\fontsize{14pt}{17pt}\selectfont}
\renewcommand{\cftafterlottitle}{\hfill}
\setlength{\cftbeforelottitleskip}{-22pt} % Pull up to match Chapter top
...
% \onehalfspacing % This is approx 1.25x, often looked too narrow compared to Word
\setstretch{1.5} % Manually set to 1.5x to match Word's "1.5 lines" visual
...
\lstdefinestyle{code}{
  ...
  basicstyle=\ttfamily\small\color{black}\singlespacing,
...
Metode yang digunakan adalah \textit{Research and Development} (R\&D) dengan pendekatan \textit{prototyping} iteratif. Evaluasi dilakukan melalui skenario tugas representatif. Hasil menunjukkan bahwa arsitektur \textit{Single-Shot Intelligence} dengan pembatasan perubahan berbasis \textit{diff} memudahkan pengembangan bertahap dan menunjukkan efisiensi waktu dibandingkan pendekatan tradisional.
...
We adopt a Research and Development approach with iterative prototyping. Evaluation uses representative programming scenarios. Results indicate that \textit{Single-Shot Intelligence} with \textit{diff}-based change constraints facilitates incremental development and proves more efficient than traditional multi-call approaches.
...
% Spacing: 1.0 within entry (single), Gap between entries corresponds to 1.5 spacing (approx 0.5 baseline + 1.0 text)
