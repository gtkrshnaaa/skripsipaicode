...
Arsitektur \textit{Single-Shot Intelligence} mengoptimalkan efisiensi dengan menggunakan tepat 2 panggilan API: (1) fase perencanaan untuk analisis mendalam dan perencanaan komprehensif, dan (2) fase eksekusi untuk implementasi cerdas dengan adaptasi dinamis. Sistem mencakup klasifikasi intensi (\textit{chat} vs \textit{task}), manajemen API key tunggal dengan migrasi otomatis dari sistem multi-key, \textit{interrupt handling} (Ctrl+C), dan pencatatan sesi ke \texttt{.pai\_history}.

Metode yang digunakan adalah \textit{Research and Development} (R\&D) dengan pendekatan \textit{prototyping} iteratif. Evaluasi dilakukan melalui skenario tugas representatif, dengan metrik efisiensi (jumlah panggilan API), ketepatan hasil (kompilasi/eksekusi), dan kepatuhan keamanan \textit{path}. Hasil menunjukkan bahwa agen \textit{stateful} dengan arsitektur \textit{Single-Shot Intelligence} dan pembatasan perubahan berbasis \textit{diff} memudahkan pengembangan bertahap sambil menekan risiko penimpaan berkas. Sistem 2-panggilan API terbukti lebih efisien dibandingkan pendekatan tradisional yang memerlukan 10-20 panggilan API, dengan tetap mempertahankan kualitas hasil yang optimal.
...
The \textit{Single-Shot Intelligence} architecture optimizes efficiency by using exactly 2 API calls: (1) planning phase for deep analysis and comprehensive planning, and (2) execution phase for intelligent implementation with dynamic adaptation. The system includes intent classification (\textit{chat} vs \textit{task}), single API key management with automatic migration from multi-key systems, \textit{interrupt handling} (Ctrl+C), and session logging to \texttt{.pai\_history}.

We adopt a Research and Development approach with iterative prototyping. The evaluation uses representative programming scenarios and measures efficiency (API call count), correctness (build/run), and security compliance. Results indicate that a stateful agent with \textit{Single-Shot Intelligence} and \textit{diff}-based change constraints facilitates incremental development while reducing the risk of unintended overwrites. The 2-call API system proves more efficient than traditional approaches requiring 10-20 API calls, while maintaining optimal result quality.
