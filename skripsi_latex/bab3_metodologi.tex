...
Penelitian ini menggunakan pendekatan \textit{Research and Development} (R\&D) dengan strategi \textit{prototyping} iteratif. Pemilihan metode ini didasarkan pada beberapa pertimbangan:

\begin{enumerate}
    \item \textbf{Eksplorasi Desain Agen Stateful}: Berbeda dengan aplikasi konvensional yang bersifat \textit{stateless}, agen AI memerlukan manajemen konteks percakapan dan memori kerja yang kompleks. Pendekatan \textit{prototyping} memungkinkan eksperimen cepat terhadap berbagai strategi manajemen state (misalnya, ukuran context window, format log sesi) tanpa komitmen arsitektur jangka panjang.
    
    \item \textbf{Validasi Asumsi Keamanan}: Kebijakan \textit{path security} dan pembatasan \textit{diff} merupakan mekanisme novel yang belum teruji di konteks agen CLI. Siklus iteratif memungkinkan identifikasi edge case (seperti symbolic links, path traversal attacks) melalui pengujian langsung, yang sulit diprediksi hanya dari analisis teoritis.
    
    \item \textbf{Optimasi Efisiensi Token}: Arsitektur \textit{Single-Shot Intelligence} dikembangkan melalui iterasi bertahap—dimulai dari model \textit{chat-loop} konvensional (10-20 panggilan API per tugas), kemudian dipadatkan menjadi sistem 2-panggilan melalui eksperimen empiris terhadap berbagai strategi prompt engineering.
\end{enumerate}

\subsection{Trade-off Metodologis}
Pendekatan \textit{prototyping} dipilih dibandingkan metode waterfall atau agile penuh dengan pertimbangan trade-off berikut:

\begin{itemize}
    \item \textbf{Kelebihan}: Fleksibilitas tinggi untuk mengubah desain berdasarkan temuan empiris; cocok untuk domain yang belum mature (agentic AI untuk CLI); memungkinkan validasi konsep sebelum investasi besar pada infrastruktur.
    
    \item \textbf{Kekurangan}: Dokumentasi arsitektur dapat tertinggal jika iterasi terlalu cepat; risiko \textit{scope creep} jika tidak ada batasan jelas per iterasi; potensi \textit{technical debt} jika refactoring tidak dilakukan secara disiplin.
    
    \item \textbf{Mitigasi}: Setiap iterasi dibatasi pada satu fitur utama (misalnya, iterasi 1: path security; iterasi 2: diff-aware modification; iterasi 3: Single-Shot Intelligence); dokumentasi arsitektur diperbarui setelah setiap iterasi stabil; code review dilakukan sebelum merge ke branch utama.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Manajemen Konfigurasi (\texttt{config.py})}: menyimpan dan mengelola API key tunggal dalam format JSON di \texttt{~/.config/pai-code/credentials.json} dengan izin berkas 0o600 (read-write owner only). Validasi API key Google (harus dimulai dengan "AIza" dan minimal 20 karakter). Mendukung operasi: \texttt{set}, \texttt{show}, \texttt{remove}, \texttt{validate}, dan migrasi otomatis dari sistem multi-key.
...
\begin{longtable}{@{}p{0.25\textwidth}p{0.70\textwidth}@{}}
  ...
  Konfigurasi (\texttt{config.py}) & Manajemen API key tunggal dalam format JSON di \texttt{~/.config/pai-code/credentials.json} dengan permission 0o600. Validasi API key Google (prefix "AIza", minimal 20 karakter). Operasi: \texttt{set}, \texttt{show}, \texttt{remove}, \texttt{validate}, dan migrasi otomatis dari sistem multi-key. \\
...
\begin{itemize}
  ...
  \item Bahasa pemrograman: Python (\texttt{\textgreater= 3.10}, sesuai spesifikasi \texttt{setup.cfg}).
  ...
  \item LLM: Google Gemini (model default \texttt{gemini-2.5-flash-lite}, temperature default 0.3, dapat dikonfigurasi via \texttt{PAI\_MODEL} dan \texttt{PAI\_TEMPERATURE}) melalui paket \texttt{google-generativeai} versi $\geq$ 0.5.4.
  ...
  \item Variabel lingkungan: \texttt{PAI\_MODEL}, \texttt{PAI\_TEMPERATURE}, \texttt{PAI\_MODIFY\_THRESHOLD}, \texttt{PAI\_MODIFY\_MAX\_RATIO}, serta variabel noise suppression (\texttt{GRPC\_VERBOSITY}, \texttt{GRPC\_LOG\_SEVERITY}, \texttt{ABSL\_LOGGING\_MIN\_LOG\_LEVEL}, dll) untuk menekan log gRPC/absl yang berisik.
