...
\begin{longtable}{@{}p{0.26\textwidth}p{0.68\textwidth}@{}}
  ...
  Agen (\texttt{agent.py}) & Penyusun prompt, eksekusi rencana aksi, memori percakapan; memanggil \texttt{llm}, \texttt{fs}, \texttt{ui}. \\
  LLM Bridge (\texttt{llm.py}) & Integrasi Gemini API (\texttt{google-generativeai}); mengambil API key dari \texttt{config}. \\
  File System Gateway (\texttt{fs.py}) & Operasi berkas tingkat-aplikasi, validasi \textit{path}, blokir direktori sensitif, \textit{diff}-aware modify. \\
...
\begin{longtable}{@{}p{0.06\textwidth}p{0.18\textwidth}p{0.70\textwidth}@{}}
  ...
  6 & FS/UI & Menjalankan operasi berkas (dengan \textit{path security} dan \textit{diff}-aware) dan menampilkan hasil di terminal. \\
