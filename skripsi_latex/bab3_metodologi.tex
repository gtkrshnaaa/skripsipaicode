...
Penelitian ini menggunakan pendekatan \textit{Research and Development} (R\&D) dengan strategi \textit{prototyping} iteratif. Tahapan prosedur penelitian dilakukan sebagai berikut:
...
\begin{enumerate}
  ...
  \begin{enumerate}
      ...
      \item Mengumpulkan data operasional (waktu eksekusi, jumlah langkah) dan kualitas (keberhasilan kompilasi).
      \item Mengevaluasi keamanan dengan mencoba skenario akses ilegal ke direktori sensitif.
  \end{enumerate}
  
  \item \textbf{Analisis Hasil dan Penyusunan Laporan}: Menganalisis hasil pengujian untuk menarik kesimpulan mengenai efektivitas arsitektur yang diusulkan dan menyusun laporan akhir.
\end{enumerate}
...
% Tabel 3.1: Daftar modul dan dependensi
\begin{longtable}{@{}p{0.25\textwidth}p{0.70\textwidth}@{}}
  \caption{Daftar Modul dan Tanggung Jawab}\label{tab:diagram-modul}\\
  \toprule
  ...
  \texttt{workspace.py} & Operasi I/O berkas, penegakan kebijakan path, penerapan \textit{diff}. \\
...
% Visualisasi Flowchart Alur Sistem (Menggantikan Tabel Sequence)
\begin{figure}[htbp]
  \centering
  ...
  \end{tikzpicture}
  \caption{Flowchart alur eksekusi perintah dalam arsitektur \textit{Single-Shot} Paicode.}
