...
\section{Objek dan Bahan Penelitian}
Objek dan bahan yang digunakan dalam penelitian ini meliputi:
...
\begin{enumerate}
    ...
    \item \textbf{Instrumen Pengujian}: Skenario pengujian yang mencakup pembuatan struktur proyek, manipulasi berkas, dan refaktorisasi kode untuk mengukur kinerja agen.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item Sistem harus menyimpan riwayat percakapan untuk mempertahankan konteks antar sesi interaksi.
    \item Sistem harus memberikan mekanisme \textit{interrupt handling} (Ctrl+C) untuk menghentikan respons tanpa keluar dari sesi.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Keandalan}: Sistem harus dapat menangani error dengan graceful degradation dan memberikan pesan error yang informatif.
    \item \textbf{Portabilitas}: Sistem harus dapat berjalan pada berbagai distribusi Linux dengan dependensi minimal.
\end{enumerate}
...
Arsitektur Paicode dirancang secara modular dengan pemisahan tanggung jawab yang jelas antar komponen. Arsitektur modular ini memudahkan pengembangan, pengujian, dan pemeliharaan sistem. Komponen-komponen utama sistem adalah:
...
\begin{longtable}{@{}p{0.25\textwidth}p{0.70\textwidth}@{}}
  ...
  \midrule
  \endhead
  \texttt{cli.py} & Entry point aplikasi, parsing argumen CLI, inisialisasi sesi. \\
  \texttt{agent.py} & Logika bisnis utama, klasifikasi intensi, perencanaan terstruktur (JSON), orkestrasi eksekusi adaptif, manajemen memori percakapan. \\
  \texttt{llm.py} & Abstraksi komunikasi dengan API Gemini, manajemen token, sanitasi response. \\
  \texttt{workspace.py} & Operasi I/O berkas, penegakan kebijakan \textit{path security}, penerapan modifikasi berbasis \textit{diff} dengan threshold keamanan. \\
  \texttt{config.py} & Penyimpanan aman dan validasi API Key, manajemen konfigurasi pengguna. \\
  \texttt{ui.py} & Rendering output terminal dengan \texttt{rich}, syntax highlighting, visualisasi status. \\
  \bottomrule
\end{longtable}

\subsection{Alur Kerja Sistem}
Diagram berikut menggambarkan alur kerja sistem dalam satu sesi interaksi pengguna (satu \textit{turn}):
...
Alur kerja dimulai dengan penerimaan instruksi pengguna, kemudian LLM membuat rencana tindakan dalam format JSON terstruktur. Rencana ini divalidasi oleh sistem (termasuk pengecekan \textit{path security}). Jika valid, sistem mengeksekusi langkah-langkah dalam rencana secara berurutan dengan melakukan operasi pada workspace melalui modul \texttt{workspace.py}. Hasil eksekusi disimpan ke riwayat percakapan untuk menjaga konteks.
