...
\begin{itemize}
  ...
  \item \textbf{Agen (\texttt{agent.py})}: mengimplementasikan \textit{Single-Shot Intelligence} yang mencakup: (1) klasifikasi intensi (\textit{chat} vs \textit{task}), (2) fase perencanaan untuk analisis mendalam, dan (3) fase eksekusi untuk implementasi cerdas dengan adaptasi dinamis. Menyediakan perintah: \texttt{MKDIR}, \texttt{TOUCH}, \texttt{READ}, \texttt{WRITE}, \texttt{MODIFY}, \texttt{RM}, \texttt{MV}, \texttt{TREE}, \texttt{LIST\_PATH}, \texttt{FINISH}. Mengelola memori percakapan dengan pencatatan sesi ke \texttt{.pai\_history}.
  ...
  \item \textbf{Pengatur Workspace (\texttt{workspace.py})}: bertindak sebagai \emph{workspace controller} yang menyediakan fungsi-fungsi terpusat untuk menjalankan operasi tingkat-aplikasi pada ruang kerja proyek. Sebelum aksi dieksekusi, modul ini menegakkan kebijakan \textit{path security} (normalisasi, verifikasi akar, dan deny-list direktori sensitif). Sistem modifikasi berbasis \textit{diff} mencegah penimpaan berkas tidak diinginkan dengan atomic write menggunakan tempfile.
...
Alur data tipikal dengan \textit{Single-Shot Intelligence}: masukan pengguna (CLI) → klasifikasi intensi → konstruksi prompt (Agen) → fase perencanaan (panggilan LLM pertama) → fase eksekusi (panggilan LLM kedua) → pencatatan konteks sebagai memori percakapan.
...
\begin{longtable}{@{}p{0.25\textwidth}p{0.70\textwidth}@{}}
  ...
  Agen (\texttt{agent.py}) & Implementasi \textit{Single-Shot Intelligence}: klasifikasi intensi, fase perencanaan, dan fase eksekusi dalam 2 panggilan API. Mengelola memori percakapan, \textit{interrupt handling} (Ctrl+C), dan pencatatan sesi ke \texttt{.pai\_history}. Memanggil \texttt{llm}, \texttt{workspace}, \texttt{ui}. \\
  ...
  Pengatur Workspace (\texttt{workspace.py}) & \emph{Workspace controller} dengan fungsi operasi workspace (baca/tulis, buat/hapus/pindah, tree/list path). Sistem modifikasi berbasis \textit{diff} dengan atomic write. Penegakan \textit{path security} (normalisasi, verifikasi akar, deny-list). \\
...
\begin{longtable}{@{}p{0.06\textwidth}p{0.18\textwidth}p{0.70\textwidth}@{}}
  ...
  4 & Agen & Memberikan respons awal singkat untuk mengakui permintaan pengguna. \\
  5 & LLM & \textbf{Fase Perencanaan} (Panggilan API 1): Melakukan analisis mendalam dan menghasilkan perencanaan komprehensif. \\
  6 & Agen & Memberikan respons sebelum eksekusi untuk menunjukkan kesiapan melaksanakan rencana. \\
  7 & LLM & \textbf{Fase Eksekusi} (Panggilan API 2): Implementasi cerdas dengan adaptasi dinamis berdasarkan rencana. \\
...
\begin{itemize}
  ...
  \item Variabel lingkungan: \texttt{PAI\_MODEL}, \texttt{PAI\_TEMPERATURE}, dan variabel untuk menekan log gRPC/absl yang berisik.
